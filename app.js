...
{
  const acc_no = agent.parameters.acc_no;
  const client = new MongoClient(url);
  await client.connect();
  const snap = await client.db("chatbot").collection("user_info").findOne({acc_no: acc_no});
...
function report_issue(agent)
{
  ...
  const intent_val=agent.parameters.issue_no;
  ...
  var dbo = db.db("chatbot");
    ...
    dbo.collection("issue_info").insertOne(myobj, function(err, res) {
    if (err) throw err;
    db.close();    
  });
 });
